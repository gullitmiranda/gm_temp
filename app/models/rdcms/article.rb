...
module Rdcms
  class Article < ActiveRecord::Base
    ...
    has_many        :metatags
    has_many        :images, :through => :article_images, :class_name => Rdcms::Upload
    ...
    has_many        :comments, :class_name => Rdcms::Comment
    ...
    if ActiveRecord::Base.connection.table_exists?("rdcms_settings")
      if Rdcms::Setting.for_key("rdcms.use_solr") == "true"
        searchable do
          text :title, :boost => 5
          text :summary
          text :content
    ...
    def comments_of_subarticles
      Rdcms::Comment.where("article_id in (?)", self.subtree_ids)
    end
    ...
    def self.init_image_methods
      if ActiveRecord::Base.connection.table_exists?("rdcms_settings")
        Rdcms::Setting.for_key("rdcms.article.image_positions").split(",").each do |image_type|
          define_method "image_#{image_type.underscore}" do
            self.image(image_type,"original")
          end
          Rdcms::Upload.attachment_definitions[:image][:styles].keys.each do |style_name|
    ...
    Rdcms::Article.init_image_methods
    ...
    def absolute_public_url
      "http://#{Rdcms::Setting.for_key('rdcms.url')}#{self.public_url}"
    end
    ...
    def verify_existence_of_opengraph_image
      if Rdcms::Metatag.where("article_id = ? AND name = 'OpenGraph Image'", self.id).count == 0
        Rdcms::Metatag.create(article_id: self.id, name: "OpenGraph Image", value: Rdcms::Setting.for_key("rdcms.facebook.opengraph_default_image"))
      end
    end
    ...
    def mark_as_startpage!
      Rdcms::Article.startpage.each do |a|
        a.startpage = false
        a.save
      end
      self.startpage = true
    ...
    def self.active
      Rdcms::Article.where("active = 1 AND active_since < '#{Time.now.utc}'")
    end

    def self.search_by_url(url)
      article = nil
      articles = Rdcms::Article.where(:url_name => url.split("/").last.to_s.split(".").first)
    ...
    def self.recent(count)
      Rdcms::Article.where('title IS NOT NULL').order('updated_at DESC').limit(count)
    end
