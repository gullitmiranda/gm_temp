...
module Rdcms
  class Upload < ActiveRecord::Base
    ...
    if ActiveRecord::Base.connection.table_exists?("rdcms_uploads")
    ...
    has_many :article_images, :class_name => Rdcms::ArticleImage
    has_many :articles, :through => :article_images
    has_many :imports, :class_name => Rdcms::Import
    belongs_to :attachable, polymorphic: true
    has_and_belongs_to_many :products, :join_table => "rdcms_products_uploads"
      ...
      if self.upload_file_name.include?(".zip") && File.exists?(self.image.path)
        ...
        zipped_files.each do |zipped_file|
          ...
          if zipped_file.file?
            ...
            Rdcms::Upload.create(:image => File.open("tmp/#{self.id}_unzipped_#{int}.jpg"),
                        :source => self.source,
                        :rights => self.rights,
