jQuery ->
  #### Selectores
  sliders_container  = $('.fileuploads-container')
  file_selector           = $('#file_selector', sliders_container)
  auto_save               = $('#auto_save', file_selector)
  auto_save_checkbox      = $(':checkbox', auto_save)
  fileupload_form         = $('#fileupload:first'     , sliders_container)
  upload_container_ul     = $('#upload_container ul:first'  , sliders_container)
  ...
  _checkOrCreateViewPort = ->
    viewPort = $('body .slider_edit_viewport')
    viewPort = if viewPort.length then viewPort else $("""
      <div class="slider_edit_viewport fade in overlay-viewport">
        <div class="overlay-block overlay-white"></div>
        <div class="slider_edit_container overlay-container"><div class="container"></div></div>
      </div>
""").appendTo($("body").css("overflow", "hidden"))
  ...
  # console.debug upload_container_ul, sliders_itens
  $(document).on "click", "#{sliders_itens.selector} .edit .btn", _renderEdit
  # end Callbacks
