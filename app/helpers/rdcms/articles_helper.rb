module Rdcms
  module ArticlesHelper
    ...
    def render_article_content_parts(article)
      render :partial => "/rdcms/articles/show", :locals => {:article => article}
    end

    def render_article_image_gallery
      if @article
        result = ""
        uploads = Rdcms::Upload.tagged_with(@article.image_gallery_tags.present? ? @article.image_gallery_tags.split(",") : "" )
        if uploads && uploads.count > 0
          uploads.each do |upload|
            result << content_tag("li", link_to(image_tag(upload.image.url(:thumb)), upload.image.url(:large)))
          end
        end
        return content_tag("ul", raw(result), :class => "rdcms_article_image_gallery")
      end
    end
    ...
    def index_of_articles(options={})
      if @article && @article.article_for_index_id.present? && master_index_article = Rdcms::Article.find_by_id(@article.article_for_index_id)
        result_list = ""
    ...
    def render_article_widgets(options={})
      ...
      if @article
        ...
        if tags.present? && default == "false"
          ...
          widgets = Rdcms::Widget.where(:default => true).tagged_with(tags.split(","))
        else
    ...
    def navigation_menu_helper(child, depth, current_depth, options)
      ...
      child_link = child_link + content_tag("div", raw(template.render(Rdcms::Article::LiquidParser)), :class => "navigtion_link_description") unless options[:show_description] == false
